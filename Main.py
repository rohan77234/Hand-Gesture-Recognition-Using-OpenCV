            ...
            if fingers[-1] == 1:
                ...
                print(mode)
                 ...
                 #   print(lmList[4], lmList[8])
                    ...
                    if volPer > 0 and volPer <= 100:
                        volBar = np.interp(volPer, [0, 100], [400, 150])
                        volPer = str(int(volPer)) + '%'
                        cv2.putText(img, volPer, (50, 50), cv2.FONT_HERSHEY_COMPLEX, 1, color, 2)
...
################## Cursor 👇👇👇##################
    if mode == 'Cursor':
        ...
        putText(mode)
        if len(lmList) != 0:
            x3 = np.interp(lmList[8][1], [0, wCam], [0, autopy.screen.size()[0]])
            y3 = np.interp(lmList[8][2], [0, hCam], [0, autopy.screen.size()[1]])
            autopy.mouse.move(x3, y3)
            cv2.circle(img, (lmList[8][1], lmList[8][2]), 15, color, cv2.FILLED)

################## Screenshot 👇👇👇##################
    if fingers == [0, 1, 0, 1, 1]:
        putText(mode = 'Screenshot', loc=(50, 50), color = (0, 215, 255))
        active = 0
